...
def load_center_partition():
    ...
    df = load_data()
    datasets = create_tensor_datasets(df, 'country', 5)

    # Directly use 'idx' to get the corresponding datasets
    ...
    con_train = ConcatDataset(train_datasets)
    con_val = ConcatDataset(val_datasets)
    con_test = ConcatDataset(test_datasets)

    return con_train, con_val, con_test
