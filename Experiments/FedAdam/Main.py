...
import flwr as fl
from utils import load_partition, get_model_params, PointWiseModel, AdaptiveDualBranchNet
...
def get_non_extension_params(model):
    # Filter out parameters that are part of the extension branch
    non_extension_params = {name: param for name, param in model.state_dict().items() if 'extension_branch' not in name}
    return non_extension_params

def get_model_params_test(hidden, hidden_ext, dropout, num_layers, training_size):
    model = AdaptiveDualBranchNet(hidden, hidden_ext, dropout, num_layers, training_size)
    non_extension_params = get_non_extension_params(model)
    return [val.cpu().numpy() for val in non_extension_params.values()]
...
def main(cfg: DictConfig):
    ...
    for run in range(N_RUNS):
        ...
        strategy = SaveModelStrategy(
            ...
            eta = cfg.federation.eta,
            eta_l = cfg.federation.eta_l,
            beta_1 = cfg.federation.beta_1,
            beta_2 = cfg.federation.beta_2,
            tau = cfg.federation.tau, 
            run_number=run_number,
            initial_parameters=fl.common.ndarrays_to_parameters(
                                         get_model_params_test(cfg.model.hidden, cfg.model.hidden_ext,
                                                               cfg.model.dropout, cfg.model.num_layers, 2000))
                                     )
